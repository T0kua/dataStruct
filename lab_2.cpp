...
int main() {
	...
	// Îïðåäåëÿåì ìàòðèöû A, B è C
	std::complex<double> A[N][N], B[N][N], C[N][N];

	// Èíèöèàëèçàöèÿ ìàòðèö A è B ñëó÷àéíûìè çíà÷åíèÿìè
	for (int i = 0; i < N; ++i) {
		for (int j = 0; j < N; ++j) {
			A[i][j] = std::complex<double>(rand() % 10, rand() % 10); // Ñëó÷àéíûå êîìïëåêñíûå ÷èñëà
			B[i][j] = std::complex<double>(rand() % 10, rand() % 10);
			C[i][j] = std::complex<double>(0, 0); // Èíèöèàëèçàöèÿ C íóëÿìè
		}
	}

	// Ïàðàìåòðû äëÿ cblas_zgemm
	const double alpha = 1.0; // Êîýôôèöèåíò äëÿ A*B
	const double beta = 0.0;   // Êîýôôèöèåíò äëÿ C
